...
{
	$sql = "INSERT INTO `status` (name, title, color, customCSS, since, until, accesses, icon) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
	$stmt = $db->prepare($sql);
	if ($stmt) {
		$stmt->bind_param("ssssssss", $name, $data['title'], $data['color'], $data['customCSS'], $data['since'], $data['until'], $data['accesses'], $data['icon']);
		$stmt->execute();
