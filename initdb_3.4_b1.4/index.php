...
echo 'creating table status: id | name | title | color | customCSS | levelLink | accesses | icon<br>';
...
$sql = 'INSERT INTO `status` (name, title, color, customCSS, accesses, icon, levelLink) VALUES (?, ?, ?, ?, ?, ?, ?)';
$stmt = $db->prepare($sql);
...
foreach ($statuses as $name => $data) {
    $stmt->bind_param('ssssssi', $name, $data['title'], $data['color'], $data['customCSS'], $data['accesses'], $data['icon'], $data['levelLink']);
